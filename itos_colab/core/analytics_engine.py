"""
Phase 2 - Strategy Analytics Engine (Google Colab Edition)
==========================================================
...
from .config import ANALYTICS_DIR, LOGS_DIR, MIN_TRADES, MAX_DRAWDOWN, MIN_PROFIT_FACTOR, COLAB_SETTINGS
...
class ColabAnalyticsEngine:
    """
    Google Colab optimized strategy analytics and evaluation engine.
    ...
    def __init__(self, analytics_dir: str = ANALYTICS_DIR):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(os.path.join(LOGS_DIR, 'analytics_engine.log')),
        ...
        logger.info("🚀 ColabAnalyticsEngine initialized")
    ...
    def analyze_strategy(self, strategy_name: str, trades_df: pd.DataFrame) -> StrategyResult:
        ...
        if trades_df.empty:
            logger.warning(f"⚠️ No trades found for {strategy_name}")
    ...
    def _analyze_yearly_performance(self, trades_df: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            ...
            logger.error(f"❌ Error analyzing yearly performance: {e}")
            return {}
    ...
    def _save_analytics(self, result: StrategyResult):
        """Save analytics results to Google Drive."""
        try:
            ...
            # Save summary
            summary_file = os.path.join(self.analytics_dir, f"{result.strategy_name}_summary.csv")
            summary_df.to_csv(summary_file, index=False)
            
            # Save detailed analytics
            analytics_file = os.path.join(self.analytics_dir, f"{result.strategy_name}_analytics.json")
    ...
    def _generate_plots(self, result: StrategyResult, trades_df: pd.DataFrame):
        """Generate visualization plots and save to Google Drive."""
        try:
            if trades_df.empty:
                return
            ...
            plot_file = os.path.join(self.analytics_dir, f"{result.strategy_name}_dashboard.png")
    ...
    def compare_strategies(self, strategy_names: List[str]) -> pd.DataFrame:
        ...
        try:
            ...
            for strategy_name in strategy_names:
                analytics_file = os.path.join(self.analytics_dir, f"{strategy_name}_analytics.json")
            ...
            comparison_df = pd.DataFrame(comparison_data).sort_values('Total PnL', ascending=False)
            
            # Save comparison
            comparison_file = os.path.join(self.analytics_dir, "strategy_comparison.csv")
            comparison_df.to_csv(comparison_file, index=False)
            
            logger.info(f"💾 Saved strategy comparison")
    ...
    def get_analytics_summary(self) -> Dict[str, Any]:
        ...
        try:
            strategies = self.list_analyzed_strategies()
            
            return {
                'analyzed_strategies': len(strategies),
    ...
    def list_analyzed_strategies(self) -> List[str]:
        ...
        try:
            ...
            strategies = []
            for file_path in os.listdir(self.analytics_dir):
                if file_path.endswith("_summary.csv"):
                    strategy_name = file_path.replace("_summary.csv", "")
                    strategies.append(strategy_name)
            
            return sorted(strategies)
        except Exception as e:
            logger.error(f"❌ Error listing analyzed strategies: {e}")
            return []